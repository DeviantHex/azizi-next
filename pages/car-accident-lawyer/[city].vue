<template>
  <VContainer v-if="detail" class="info-page" :class="{ 'px-6': smAndDown }">
    <div class="cover" :style="{ height: smAndDown ? '300px' : '500px', position: smAndDown ? 'relative' : 'sticky', top: smAndDown ? undefined : 'calc(var(--v-layout-top) + 16px)' }">
      <img class="background" :src="imageUrl" :alt="`Car Accident Lawyer ${detail.city} - The Azizi Firm`" />
      <img class="logo" src="/assets/images/banner-logo.svg" width="300" alt="The Azizi Firm Logo" />
      <h1>{{ detail.city }} <span>Car Accident Lawyer</span></h1>
    ...
    <div class="info" :class="{ 'text-center': smAndDown }">
      ...
      <div class="text-block">
        <h2 :class="{ 'text-icon-end-short': !smAndDown }">{{ detail.city }} Car Accident Attorney</h2>
      ...
      <div v-if="detail.faq && detail.faq.length" class="text-block">
        <h2 class="text-secondary">Car Accident FAQs</h2>
        <div v-for="(q, i) in detail.faq" :key="i" class="text-block">
          <h3>{{ q.question }}</h3>
          <p>{{ q.answer }}</p>
        </div>
      </div>
      <h2 class="text-secondary">Why Hire Us For Your {{ detail.city }} Car Accident Case?</h2>
      <div class="text-block">
        <h3>Experience Against Insurance Companies</h3>
        <p>{{ detail.whyHireText }}</p>
      </div>
      <div class="text-block">
        <h3>No Upfront Fees</h3>
        <p>We work on a contingency fee basis. You don't pay unless we win your car accident case.</p>
      </div>
      <div v-if="detail.nearbyCities && detail.nearbyCities.length" class="text-block">
        <h3>Also Serving Nearby Areas</h3>
        <p>In addition to {{ detail.city }}, we serve: {{ detail.nearbyCities.join(', ') }} and throughout all of California.</p>
      </div>
      <div class="text-block">
        <h3>Contact Us</h3>
...
const { data: detail } = await useAsyncData(`car-acc-${route.params.city}`, () =>
  import(`~/data/locationPages/car-accident/${route.params.city}.json`).then(m => m.default || m)
)
...
const imageUrl = computed(() => detail.value?.src || '/assets/images/car-accidents.png')
const locationInsights = computed(() => buildLocationInsightSections('car-accident-lawyer', route.params.city as string))

const pageTitle = computed(() => `${detail.value.city} Car Accident Lawyer | The Azizi Firm`)
const pageDescription = computed(() =>
  `${detail.value.city} car accident lawyer. Free consultation. Auto accidents, truck accidents, motorcycle accidents. No fees unless we win. Call (858) 829-3962.`
)
  ...
  script: [{
    ...
    children: () => JSON.stringify({
      ...
      '@graph': buildLocationSchema({
        ...
        pagePath: route.path,
        pageTitle: `${detail.value.city} Car Accident Lawyer`,
        pageDescription: pageDescription.value,
        parentName: 'Personal Injury',
        parentPath: '/practice-areas/personal-injury',
        serviceLabel: 'Car Accident Lawyer',
