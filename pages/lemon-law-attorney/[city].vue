<template>
  <VContainer v-if="detail" class="info-page" :class="{ 'px-6': smAndDown }">
    <div class="cover" :style="{ height: smAndDown ? '300px' : '500px', position: smAndDown ? 'relative' : 'sticky', top: smAndDown ? undefined : 'calc(var(--v-layout-top) + 16px)' }">
      <img class="background" :src="imageUrl" :alt="`Lemon Law Attorney ${detail.city} - The Azizi Firm`" />
      <img class="logo" src="/assets/images/banner-logo.svg" width="300" alt="The Azizi Firm Logo" />
      <h1>{{ detail.city }} <span>Lemon Law Attorney</span></h1>
    ...
    <div class="info" :class="{ 'text-center': smAndDown }">
      <div class="text-block"><PageMetaBar /><h2 :class="{ 'text-icon-end-short': !smAndDown }">{{ detail.city }} Lemon Law Lawyer</h2><p>{{ detail.introText }}</p></div>
      ...
      <template v-for="(section, i) in detail.sections" :key="i"><div class="text-block"><h3>{{ section.header }}</h3><p>{{ section.body }}</p></div></template>
      <div v-if="detail.faq && detail.faq.length" class="text-block"><h2 class="text-secondary">Lemon Law FAQs</h2><div v-for="(q, i) in detail.faq" :key="i" class="text-block"><h3>{{ q.question }}</h3><p>{{ q.answer }}</p></div></div>
      <h2 class="text-secondary">Why Hire Us For Your {{ detail.city }} Lemon Law Case?</h2>
      <div class="text-block"><h3>Song-Beverly Act Experts</h3><p>{{ detail.whyHireText }}</p></div>
      <div class="text-block"><h3>No Upfront Fees</h3><p>We handle lemon law cases on a contingency fee basis. If we win, the manufacturer pays our fees under California law.</p></div>
...
const { data: detail } = await useAsyncData(`lemon-law-${route.params.city}`, () =>
  import(`~/data/locationPages/lemon-law/${route.params.city}.json`).then(m => m.default || m)
)

if (!detail.value) throw createError({ statusCode: 404, statusMessage: 'City page not found' })

const imageUrl = computed(() => detail.value?.src || '/assets/images/lemon-law.png')
const locationInsights = computed(() => buildLocationInsightSections('lemon-law-attorney', route.params.city as string))
const pageTitle = computed(() => `${detail.value.city} Lemon Law Attorney | The Azizi Firm`)
const pageDescription = computed(() => `${detail.value.city} California lemon law lawyer. Get a refund or replacement for your defective vehicle. Free evaluation. No fees unless we win. Call (858) 829-3962.`)
...
useHead({
  script: [{ type: 'application/ld+json', children: () => JSON.stringify({
    '@context': 'https://schema.org', '@graph': buildLocationSchema({
      ...
      pagePath: route.path,
      pageTitle: `${detail.value.city} Lemon Law Attorney`,
      pageDescription: pageDescription.value,
      parentName: 'Lemon Law',
      parentPath: '/practice-areas/lemon-law',
      serviceLabel: 'Lemon Law Attorney',
